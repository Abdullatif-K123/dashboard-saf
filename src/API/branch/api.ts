...
const getAll = async (params: BranchGetAllParams, body: BranchGetAllBody) => {
  const { data } = await axiosInstance.put<Pagination<Branch>>(
...
const getSelect = async (params: BranchSelectParams) => {
  const { data } = await axiosInstance.put<BranchSelect[]>(
...
const get = async (id: string) => {
  const { data } = await axiosInstance.get<BranchDetails>(
    API_ROUTES.BRANCH.GET,
    {
      params: { id },
    }
  );
...
const changeRecord = async (params: BranchChangeRecordParams) => {
  await axiosInstance.post(API_ROUTES.BRANCH.CHANCE_RECORD_TYPE, null, {
    params,
  });
};
const remove = async (branchId: string) => {
  await axiosInstance.delete(API_ROUTES.BRANCH.REMOVE, {
    params: { branchId },
  });
};
